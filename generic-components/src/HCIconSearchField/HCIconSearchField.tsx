...
import Searcn from "@mui/icons-material/Search";
...
export const HCIconSearchField: React.FC<IconSearchFieldProps> = ({
  ...
  const [filterIcons, setFilterIcons] = useState<IconPack | null>(
    IconPack.TRUECHART
  ); // Tracks the currently selected icon pack
  ...
  function convertInlineCSSToReactStyle(
    inlineCSS: string
  ): React.CSSProperties {
    return inlineCSS
    ...
    <IconPickerDiv>
      <div className="search-iconfilters-container">
        <Tooltip
          title={
            isIconSelected
              ? "Remove the existing icon before adding a new one"
              : ""
          }
        ...
        <div className="icon-filters">
          {!isIconSelected ? (
            <>
              <button
                className={`truechat-icons bar-icon-item ${
                  filterIcons === IconPack.TRUECHART ? "active" : ""
                }`}
              ...
              <button
                className={`fontawesome-icons bar-icon-item ${
                  filterIcons === IconPack.FontAwesome ? "active" : ""
                }`}
              ...
              <button
                className={`leornado-icons bar-icon-item ${
                  filterIcons === IconPack.LeonardoUI ? "active" : ""
                }`}
            ...
            >
              <button
                className="bar-icon-item remove-icon"
      ...
      {icons.length > 0 && searchValue.trim().length > 0 && (
        <div className="icon-results">
          {icons.map((icon, index) => (
            ...
            >
              <HCButtonIcon
                icon={icon as HCButtonIconType}
                className="btn-icon-item"
              />
            </button>
          ))}
        </div>
