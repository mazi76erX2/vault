...
from connectors.store_data_in_kb import store_sharepoint_in_azure_kb, store_confluence_in_azure_kb, store_in_azure_kb
from app.database import get_user_session_by_id, update_session, update_user, add_session, refresh_displayed_docs, update_document
from email_utils import send_email_graph
from utils import get_plain_text_from_markdown, generate_markdown_from_info, get_info_markdown_txt
...
def upload_prev_questions(user_state):
    ...
    return gr.Dropdown(choices=questions_list, value=questions_list[0]), gr.File(interactive=False), gr.Button(interactive=False), user_state
...
def add_tag(existing_tags, new_tag):
    ...
    tags_markdown = " ".join([f"`{tag}`" for tag in existing_tags])  # Convert list to Markdown format
...
def go_to_resume_session(previous_session):
    ...
    return gr.Tabs(selected="1"), gr.Chatbot(json.loads(current_session['chatbot_conversation'])), gr.Textbox(current_session['summary']), gr.Textbox(current_session['tags']), gr.Markdown(value=" ".join([f"`{tag}`" for tag in json.loads(json_compatible_str)])), gr.Textbox(current_session['designated_employee']), gr.Textbox(current_session['url_reference']), gr.Textbox(current_session['document_title']), gr.Dropdown(value=current_session['validation_responsible']), gr.Dropdown(value=current_session['severity_levels'])


def go_initial_info():
    return gr.Tabs(selected="login"), gr.Dropdown(choices=[]), gr.Textbox(value=""), gr.Textbox(value=""), gr.Textbox(value=""), gr.Textbox(value=""), gr.File(value=None), gr.Textbox(value=""), gr.Textbox(value="")
...
def go_to_chatbot_tab(initial_question, state):
    ...
    current_session = get_user_session_by_id(current_session_id)[0]
...
def go_to_questions(coworker_name, coworker_crt_role, coworker_dmn_expertise, coworker_yrs_exp):
    ...
    update_info = update_user(user_glb['id'], new_name=coworker_name, current_role=coworker_crt_role, expertise_domain=coworker_dmn_expertise, experience_years=coworker_yrs_exp )
...
def on_select(evt: gr.SelectData):
    ...
    if len(mng_doc_list) > 0:
        ...
        return gr.Markdown(content), gr.Markdown(value=" ".join([f"`{tag}`" for tag in tags])), gr.Markdown(doc_info)
    else:
...
def fn_validate(info, comment):
    ...
    doc_to_store = {"file_name": selected_doc["path"], "content": selected_doc["summary"], "file_title": selected_doc["title"], "level": selected_doc["level"] }        
    ...
    return gr.Dataframe(value=mng_doc_ids_data, headers=["Documents"]), gr.Dataframe(value=verifier_doc_ids_data, headers=["Documents"]), gr.Textbox(""), gr.Markdown(''), gr.Markdown(''), gr.Markdown('')
...
def fn_validate_user(info, comment):
    ...
    update_document(selected_doc['doc_id'], status="Validated - Awaiting Approval", summary=get_plain_text_from_markdown(info), comment=comment )
    ...
    if len(mng_doc_ids_data) == 0:
        ...
        return gr.Textbox(""), gr.Markdown(''), gr.Markdown(''), gr.Markdown(''), gr.Dataframe(value=mng_doc_ids_data, headers=["Documents"]), gr.Dataframe(value=verifier_doc_ids_data, headers=["Documents"])



def modify_info(text):
    return gr.Markdown(visible=False), gr.Textbox(text, visible=True), gr.Button(visible=False), gr.Button(visible=True)

def apply_modify_info(text):
    # save the modification
    # here add a function that update the doc in db
    info_string = update_document(selected_doc['doc_id'], summary=get_plain_text_from_markdown(text))
    gr.Info(info_string, duration=1)
    
    return gr.Markdown(text, visible=True), gr.Textbox(text, visible=False), gr.Button(visible=True), gr.Button(visible=False)
...
def reject_doc(comment):
    ...
    if comment != "":
        ...
        return gr.Dataframe(value=mng_doc_ids_data, headers=["Documents"]), gr.Dataframe(value=verifier_doc_ids_data, headers=["Documents"]), gr.Textbox("")
    else:
...
def delegate_fn(comment, user):
    ...
    info_str = update_document(selected_doc["doc_id"], Reviewer=users_dict[user]["id"], status="on review", comment=comment)
    ...
    return gr.Dataframe(value=mng_doc_ids_data, headers=["Documents"]), gr.Dataframe(value=ver_doc_ids_data, headers=["Documents"]), gr.Textbox(""), gr.Dropdown(choices=[])
