...
async def create_admin_user(email: str, password: str, fullname: str, company_reg_no: str, company_name: str) -> bool:
    ...
    async with async_session_local() as db:
        ...
        await db.commit()
        print(f"Admin created: {email} profile_id={profile.id} company_id={company.id} company_reg_no={company_reg_no}")
        return True

def main() -> None:
    parser = argparse.ArgumentParser(description="Create an admin user (ensures company + profile.company_id)")
    ...
    asyncio.run(create_admin_user(args.email, args.password, args.fullname, args.company_reg_no, args.company_name))
