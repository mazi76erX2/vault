...
async def create_chat(
    ...
    try:
        ...
        new_chat = ChatMessage(user_id=user_id, message="")
        db.add(new_chat)
        await db.commit()
        ...
        return {"chatId": str(new_chat.id), "userId": str(user_id)}
...
async def get_messages(
    ...
    try:
        ...
        if chat.message:
            try:
                messages = (
                    json.loads(chat.message) if isinstance(chat.message, str) else chat.message
                )
            except Exception as e:
...
async def send_message(
    ...
    try:
        ...
        if not chat_id or not message_text:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Missing chatId or message"
            )
        ...
        if chat.message:
            try:
                ...
                logger.warning(f"Failed to parse existing messages: {e}")
                current_messages = []
