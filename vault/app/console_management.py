...
def main(args):
    get_all_users()
    with gr.Blocks(
        title=" VAULT Management Console",
        ...
        with gr.Row():
            with gr.Tabs(selected="Login") as tabs:
                ...
                with tab_doc_selection:
                    with gr.Row():
                        with gr.Column():
                            with gr.Tabs(selected="Manager"):
                                tab_doc_manager = gr.Tab("Manager Section", id="Manager")
                                with tab_doc_manager:
                                    scrolldown_docs_managers = gr.DataFrame(
                                        [],
                                        headers=["Options"],
                                        height=500,
                                        interactive=True,
                                        elem_id="scrollable_dataframe_mng",
                                    )
                                tab_doc_verifiers = gr.Tab("Expert Section", id="Verifier")
                                with tab_doc_verifiers:
                                    scrolldown_docs_verifiers = gr.DataFrame(
                                        [],
                                        headers=["Options"],
                                        interactive=True,
                                        elem_id="scrollable_dataframeverifiers",
                                        height=500,
                                    )
                        ...
                        with gr.Column():
                            ...
                            with gr.Accordion("Doc Information"):
                                ...
                                btn_apply_modify = gr.Button(
                                    "Apply the Modification", visible=False
                                )
                            ...
                            with delegate_accordion:
                                dropdown_verifier = gr.Dropdown(
                                    label="list of Verifiers", choices=[]
                                )
                                btn_delegate = gr.Button("Delegate")

                tab_connector = gr.Tab(
                    "Data Connectors (Comming Soon)", id="Connectors", interactive=False
                )
                with tab_connector:
                    with gr.Row():
                        ...
                        with gr.Column():
                            accordion_confluence = gr.Accordion(
                                "Confluence Access Data", visible=True
                            )
                            with accordion_confluence:
                                txt_url = gr.Textbox(
                                    label="Confluence URL", placeholder="Write here the url"
                                )
                                txt_user = gr.Textbox(
                                    label="Confluence Username",
                                    placeholder="Write here the username",
                                )
                                txt_keyspace = gr.Textbox(
                                    label="Confluence Keyspace",
                                    placeholder="Write here the keyspace",
                                )
                                txt_api = gr.Textbox(
                                    label="Confluence API Key",
                                    placeholder="Write here the api key",
                                    type="password",
                                )
                                btn_connect_confluence = gr.Button(value="Connect")
                            accordion_sharepoint = gr.Accordion(
                                "SharePoint Access Data", visible=False
                            )
                            with accordion_sharepoint:
                                txt_hostname_sp = gr.Textbox(
                                    label="Hostname",
                                    placeholder="For example: highcoordinationde.sharepoint.com",
                                )
                                txt_sitepath_sp = gr.Textbox(
                                    label="site path", placeholder="For example: home"
                                )
        ...
        scrolldown_docs_managers.select(
            on_select, None, [markdown_description, markdown_tags, markdown_summary]
        )
        scrolldown_docs_verifiers.select(
            on_select_verifier, None, [markdown_description, markdown_tags, markdown_summary]
        )

        btn_modify.click(
            modify_info,
            [markdown_summary],
            [markdown_summary, txt_summary, btn_modify, btn_apply_modify],
        )
        btn_apply_modify.click(
            apply_modify_info,
            [txt_summary],
            [markdown_summary, txt_summary, btn_modify, btn_apply_modify],
        )
        btn_validate.click(
            fn_validate,
            [markdown_summary, txt_comment],
            [
                scrolldown_docs_managers,
                scrolldown_docs_verifiers,
                txt_comment,
        ...
        btn_refuse.click(
            reject_doc,
            [txt_comment],
            [scrolldown_docs_managers, scrolldown_docs_verifiers, txt_comment],
        )
        btn_delegate.click(
            delegate_fn,
            [txt_comment, dropdown_verifier],
            [scrolldown_docs_managers, scrolldown_docs_verifiers, txt_comment, dropdown_verifier],
        )
        connector_dropdown.select(
            fn_connector_drop,
            [connector_dropdown],
            [accordion_confluence, accordion_sharepoint, accordion_custom],
        )
        btn_connect_confluence.click(
            fn_connect_confluence,
            [txt_url, txt_user, txt_api, txt_keyspace, txt_index],
            [txt_url, txt_user, txt_api, txt_keyspace, txt_index],
        )
        btn_connect_sharepoint.click(
            fn_connect_sharepoint,
            [txt_hostname_sp, txt_sitepath_sp, txt_index],
            [txt_hostname_sp, txt_sitepath_sp, txt_index],
        )
