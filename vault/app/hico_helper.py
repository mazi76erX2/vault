...
AZURE_SEARCH_INDEX_NAME = "hicovault_sharepoint"
...
def main(args):
    with gr.Blocks(title="HICO VAULT Helper",
        ...
        with gr.Row():
            with gr.Tabs(selected="login") as tabs:
                ...
                tab_Login = gr.Tab("HICO VAULT ChatBot Helper", visible=False, id = "chatbot")
                with tab_Login:
                    with gr.Row():
                        with gr.Column():
                                chatbot = gr.Chatbot()
                                msg = gr.Textbox(label="Search", placeholder="What is meant by BPA in the Power BI scope?")
                                with gr.Row():
                                    clear = gr.ClearButton([msg, chatbot])
                                    bt_thumbs_up = gr.Button('👍')
                                    bt_thumbs_down = gr.Button('👎')
                        with gr.Column():
                            Confidence = gr.HighlightedText(label="Confidence", combine_adjacent=True, color_map={"Low Confidence": "red", "Access Denied": "red",
                                                                                                                                    "High Confidence": "green",
                                                                                                                        "Moderate Confidence": "orange"
                                                                                                                                    })
                            as_accordion = gr.Accordion("Advanced Settings", open=False)
                            with as_accordion:
                                temperature = gr.Radio(["Creative", "Balance", "Precise"], label= "Conversation Style", value="Precise")
                                max_token = gr.Slider(label= "Max Tokens", minimum=100, maximum=1000, value=500)
                                retrieved_docs = gr.Slider(label= "Top N Results as Context", minimum=1, maximum=5,step=1, value=2)
                                llm_config = gr.Dropdown(choices=["GPT-3.5 Turbo", "GPT-4.0"], label="Large Language Model", value="GPT-3.5 Turbo")
                            sc_accordion = gr.Accordion("Search Contexts", open=False)
                            with sc_accordion:
                                gr_md = gr.Markdown(mark_text)
                            rlhw_accordion = gr.Accordion("Reinforcement Learning Human Feedback (in Preview)", visible=False)
                            with rlhw_accordion:
                                with gr.Column():
                                    report = gr.Textbox(label= "RL Human Feedback", placeholder="Write here a comment")
                                    bt_report = gr.Button('🖹 Report')
                            q_a_accordion = gr.Accordion("Reinforcement Learning Human Feedback (in Preview)", visible=False)
                            with q_a_accordion:
                                with gr.Column():
        ...
        bt_thumbs_up.click(fn_thumbs_up_rlhf, [chatbot], [question, answer, q_a_accordion, as_accordion, sc_accordion])
        msg.submit(generate_response_helper, [msg, chatbot, temperature, max_token, llm_config, retrieved_docs], [msg, chatbot, Confidence, gr_md])
        login_button.click(
            fn=login_helper,
...
if __name__=="__main__":
    ...
    parser.add_argument("--threshold", type=float, default=[0.7, 0.9], help="threshold for confidence.")
