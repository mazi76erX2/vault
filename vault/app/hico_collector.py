...
def main(args):
    ...
    with gr.Blocks(
                ...
                with tab_info:
                    with gr.Row():
                        ...
                        **To get started, please provide the following details:**
                        ...
                        upload_doc = gr.File(
                            label="Upload a File (CV or Job Description)",
                            file_types=[".txt", ".pdf", ".docx"],
                        )
                    with gr.Row():
                        coworker_name = gr.Textbox(label="Your Name", placeholder="Write your Name")
                        coworker_crt_role = gr.Textbox(
                            label="Your Current Role", placeholder="Write your current role"
                        )
                        coworker_dmn_expertise = gr.Textbox(
                            label="Your Domain of Expertise",
                            placeholder="Write your domain of expertise",
                        )
                        coworker_yrs_exp = gr.Textbox(
                            label="Years of Experience ",
                            placeholder="provide your years of experience",
                        )
                    with gr.Row():
                    ...
                    with gr.Row():
                        with gr.Accordion(label="Previous Session"):
                            sessions_dropdown = gr.Dropdown(
                                choices=[], label="Previous Sessions", interactive=True
                            )
                        ...
                        with gr.Column():
                            ...
                            upload_question_catalogue = gr.File(
                                label="Select a Question Catalogue", file_types=[".json"]
                            )

                            questions = gr.Dropdown(
                                choices=[""], label="Suggested questions", interactive=True
                            )
                    ...
                    with gr.Row():
                        ...
                        with gr.Row():
                            chat_history = gr.Textbox(
                                label="Extracted Information", value="", interactive=True
                            )
                            with gr.Column():
                                radio_check = gr.Radio(
                                    label="Validate with the Knowledge Base",
                                    choices=["Yes", "No"],
                                    value="No",
                                )
                                accordion_validation = gr.Accordion(open=False, visible=False)
                                with accordion_validation:
                                    txtbox_validation = gr.Textbox(
                                        label="Check the answer", visible=False, interactive=True
                                    )
                ...
                with tab_metadata:
                    with gr.Row():
                        ...
                        with gr.Column():
                            tags = gr.Textbox(
                                label="Tags",
                                placeholder="Add tags or keywords related to the shared knowledge.",
                                interactive=True,
                                visible=False,
                            )
                            reference_employee = gr.Textbox(
                                label="Designated Employee Contact",
                                placeholder="Add the designated employee(s) contact(s).",
                                interactive=True,
                            )
                            reference_links = gr.Textbox(
                                label="Source/Information Reference",
                                placeholder="Add the link to the source information.",
                                interactive=True,
                            )
                            title = gr.Textbox(
                                label="Document title",
                                placeholder="Choose the Doc Title",
                                interactive=True,
                            )
                            ...
                            responsible = gr.Dropdown(
                                label="Validation Responsible",
                                choices=list(managers_dict.keys()),
                                interactive=True,
                            )
                    with gr.Row():
        ...
        submit_msg.click(
            update_conversation, [msg, chatbot, state], [msg, chatbot, txtbox_validation]
        )
        btn_next_step.click(
            go_to_questions,
            [coworker_name, coworker_crt_role, coworker_dmn_expertise, coworker_yrs_exp, state],
            [tabs],
        )
        btn_previous_session.click(
            go_to_resume_session,
            [sessions_dropdown, state],
            [
                tabs,
                chatbot,
                chat_history,
        ...
        btn_generate_init_questions.click(
            generate_init_questions,
            [
                coworker_name,
                coworker_crt_role,
                coworker_dmn_expertise,
                coworker_yrs_exp,
                upload_doc,
                state,
            ],
        ...
        btn_generate_summary.click(
            generate_summary_chat, [chatbot, radio_check, state], [chat_history, txtbox_validation]
        )
        ...
        add_button.click(
            fn=add_tag,
            inputs=[tags, new_tag_input, state],
            outputs=[tags_display, tags, new_tag_input],
        )
        radio_check.change(
            radio_change, [radio_check, state], [txtbox_validation, accordion_validation]
        )
        upload_question_catalogue.change(
            initial_question_catalogue, [upload_question_catalogue, state], [questions]
        )
        login_button.click(
            fn=login_collector,
            ...
            outputs=[
                ...
                coworker_yrs_exp,
                sessions_dropdown,
                btn_previous_session,
            ],
        )
        btn_upload_previous_info.click(
            upload_prev_questions,
            [state],
            [questions, upload_question_catalogue, btn_generate_init_questions],
        )
