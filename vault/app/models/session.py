    ...
    def get_question_by_index(self, index: int) -> dict | None:
        ...
        if self.questions and 0 <= index < len(self.questions):
            return {
                ...
                "status": self.status[index] if self.status and index < len(self.status) else "Not Started",
                "topic": self.topics[index] if self.topics and index < len(self.topics) else "General",
            }
        return None