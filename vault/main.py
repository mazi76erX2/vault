...
from app.database import async_engine, Base, get_async_db
from app.email_service import send_welcome_email
from app.logger_config import setup_logging
from app.middleware.auth import verify_token, verify_token_with_tenant, get_user_company_id
...
async def get_departments(db: AsyncSession = Depends(get_async_db)):
    ...
    result = await db.execute(
        "SELECT unnest(enum_range(NULL::department))::text as value"
    )
...
async def console_main_get_user_info(
    user_id: str,
    db: AsyncSession = Depends(get_async_db)
):
    ...
    try:
        ...
        for profile in profiles:
            ...
            user_data = {
                ...
                "is_validator": profile.is_validator or (role_name_to_id_map.get("Validator") in assigned_role_ids),
    ...
    try:
        ...
        stmt = select(Company).where(
            and_(
                ...
                Company.company_reg_no == company_reg_no
            )
        )
        result = await db.execute(stmt)
        ...
        return {
            "company": {
                "id": str(company.id),
                "name": name,
                "registered_since": company.registered_since.isoformat() if company.registered_since else ""
            }
        }
    except HTTPException:
        raise
...
async def update_user_details(
    ...
    try:
        ...
        if db_user_id and requested_roles is not None:
            ...
            await db.commit()
            logger.info(f"Assigned new roles to user ID: {db_user_id} in tenant {company_reg_no} - Roles: {requested_roles}")
...
async def get_company_theme_settings(
    request: GetCompanyThemeSettingsRequest,
    db: AsyncSession = Depends(get_async_db)
):
    ...
    try:
        ...
        if not profile or not profile[0]:
            logger.warning(f"No profile or company ID found for user ID: {user_id}, returning defaults.")
        ...
        transformed_data = {
            "id": str(company.id),
            "name": company.name,
            "userChatBubbleColor": ensure_color_has_hash(company.user_chat_bubble_colour or "#007bff"),
            "botChatBubbleColor": ensure_color_has_hash(company.bot_chat_bubble_colour or "#e5e5ea"),
