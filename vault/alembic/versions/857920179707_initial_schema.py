...
Revision ID: 857920179707
Revises: 
Create Date: 2026-01-06 05:11:22.193843
...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_companies_company_reg_no'), 'companies', ['company_reg_no'], unique=True)
    op.create_index(op.f('ix_companies_contact_email'), 'companies', ['contact_email'], unique=False)
    ...
    op.create_table('chatmessagescollector',
    sa.Column('id', sa.UUID(), nullable=False),
    sa.Column('user_id', sa.UUID(), nullable=True),
    sa.Column('message', sa.Text(), nullable=False),
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    ...
    op.create_index(op.f('ix_chatmessagescollector_user_id'), 'chatmessagescollector', ['user_id'], unique=False)
    ...
    sa.Column('severity_levels', sa.Enum('PUBLIC', 'LOW', 'MEDIUM', 'HIGH', 'CRITICAL', name='security_level'), nullable=True),
    sa.Column('status', sa.Enum('DRAFT', 'PENDING', 'ON_REVIEW', 'REJECTED', 'VALIDATED_STORED', 'VALIDATED_AWAITING', name='status_enum'), nullable=True),
    sa.Column('embedding', pgvector.sqlalchemy.vector.VECTOR(dim=768), nullable=True),
    sa.Column('reviewer', sa.UUID(), nullable=True),
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
    sa.ForeignKeyConstraint(['reviewer'], ['profiles.id'], ),
    ...
    sa.Column('id', sa.UUID(), nullable=False),
    sa.Column('user_id', sa.UUID(), nullable=True),
    sa.Column('status', sa.Enum('NOT_STARTED', 'STARTED', 'IN_PROGRESS', 'COMPLETED', name='session_status_enum'), nullable=True),
    ...
    op.create_index(op.f('ix_user_roles_company_reg_no'), 'user_roles', ['company_reg_no'], unique=False)
    ...
    sa.Column('assigned_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.Column('status', sa.Enum('PENDING', 'IN_PROGRESS', 'COMPLETED', name='document_status'), nullable=True),
