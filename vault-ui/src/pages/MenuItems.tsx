...
import {ContactPage, ExpandLess, ExpandMore, TableChart, ColorLensOutlined} from '@mui/icons-material';
...
export const MenuListItems = ({open}: MenuListItemsProps) => {
    ...
    const menuItems: MenuItem[] = [
        ...
        // Adding Hico Applications menu with subpages
        {
            title: 'HICO Applications',
    ...
    return (
        <>
            {menuItems.filter((item) => {
                ...
                return (
                    <MenuListItem activeIndex={item.indexChild} item={item} key={index} showText={open}/>
                );
            })}
        </>
    );
...
const MenuListItem = ({
    ...
    const [activeIndex, setActiveIndex] = React.useState<string | undefined>(pathname);
    ...
    return (
        <Box style={{
            ...
            <ListItemButton
                onClick={() => {
                    if (onActiveIndexChanged) onActiveIndexChanged(item.indexChild ?? item.to);
            ...
            {item.subMenu && (
                <Collapse in={isActive} timeout="auto" unmountOnExit>
                    <List component="div" disablePadding>
                        {item.subMenu.map((subMenu, index) => <MenuListItem parent={item}
                            onActiveIndexChanged={(active) => setActiveIndex(active)}
                            activeIndex={activeIndex} subItem
                            item={subMenu} showText={showText}
                            key={index}/>)}
                    </List>
                </Collapse>
            )}
