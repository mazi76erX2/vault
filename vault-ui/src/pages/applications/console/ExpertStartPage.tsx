...
const ExpertStartPage: React.FC = () => {
    ...
    const [rows, setRows] = useState<DocumentToReviewRow[]>([]);
    ...
    useEffect(() => {
        const fetchDocumentsToReview = async () => {
            try {
                setLoading(true);
                ...
                const response = await axios.get<DocumentToReviewRow[]>(
                    `${VAULT_API_URL}/api/v1/expert/get-documents`,
                    { headers: {
                ...
                showError({
                    message: axios.isAxiosError(err)
                        ? (err.response?.data.detail ?? 'Error fetching documents')
    ...
    // Handle Resume
    const handleGoToDoc = (rowData: { row: DocumentToReviewRow }) => {
        console.log(rowData);
        if (!rowData.row.id) {
    ...
    return (
        <Container>
            ...
            <DancingBotGridComponent botState={'default'}>
                {/* Header */}
                <HeaderContainer>
                    <WelcomeText>Documents to review</WelcomeText>
                </HeaderContainer>
                        ...
                        <HCDataTable
                            actions={{
                                resume: {
                                    ...
                                    onClick: (row) => handleGoToDoc(row as { row: DocumentToReviewRow }),
                                },
                            }}
                            columns={columns}
                            rows={rows}
