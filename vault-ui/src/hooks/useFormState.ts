...
import { ValidateJSSchema, ValidateFunctionValidator } from '../typings/validatejs';
...
export const useFormState = <T extends NonNullable<unknown>>({schema, ...props}: FormStateProps<T>): FormState<T> => {
    const [formState, setFormState] = useState<FormStateValues<T>>({
        isLoading: false,
        isValid: false,
        values: {...props?.initialState?.values} as unknown as T,
        touched: { ...props?.initialState?.touched } as { [Property in keyof T]: boolean },
        errors: { ...props?.initialState?.errors } as { [Property in keyof T]: string[] },
    });
    ...
    const handleErrorUpdates = () => {
        ...
        const errors = validate(formState.values, schema) as { [Property in keyof T]: string[] };
    ...
    return {
        handleChange, handleValueChange, hasError, handleClear, formState: {
            ...formState,
            values: {
                ...formState.values,
            } as T
