...
export function UserListTable({
  ...
  const actions: DataTableActions = React.useMemo(() => {
    const actionsObject: DataTableActions = {
      edit: {
        icon: <HCIcon icon="Edit" style={{ color: theme.textColor.black }} />,
        onClick(params) {
      ...
      delete: {
        icon: <HCIcon icon="Trash" style={{ color: theme.error.hex }} />,
        onClick(params) {
   ...
   */
  const filteredCols = React.useMemo(() => columns, [columns]); // or filter out 'actions' if needed
