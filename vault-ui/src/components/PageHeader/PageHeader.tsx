...
import {HCButton, HCButtonProps, HCHeaderLabel, HCHeaderLabelProps} from 'generic-components';
...
export function PageHeader(props: PageHeaderProps) {
    ...
    return (
        <Box sx={{
            ...
            <HCHeaderLabel title={title.toUpperCase()} typographyProps={{
            ...
            {actions && actions.length > 0 && actions.map((action, index) => {
                ...
                return (
                    <HCButton key={index} {...buttonProps} sx={{
                        ml: 2
                    }} />
                );
            })}
