...
interface AuthContextType extends AuthState {
  // AuthState already includes userRoles
...
const authReducer = (state: AuthState, action: AuthAction): AuthState => {
  console.log(
    "[AuthContext] AuthReducer ACTION:",
    action.type,
    "PAYLOAD:",
  ...
  switch (action.type) {
    ...
    case "SET_ROLES": {
      console.log(
        "[AuthContext] Reducer SET_ROLES old state.userRoles:",
        state.userRoles,
        "new payload:",
        action.payload,
      );
      ...
      console.log(
        "[AuthContext] Reducer SET_ROLES new state.userRoles:",
        newState.userRoles,
      );
      return newState;
    }
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  console.log(
    "[AuthContext] AuthProvider rendering, current state.userRoles:",
    state.userRoles,
    "isLoadingUser:",
    state.isLoadingUser,
  );
  ...
  const fetchUserRoles = async (userId: string) => {
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data.roles)) {
        console.log(
          "[AuthContext] dispatching SET_ROLES with:",
          response.data.roles,
        );
        dispatch({ type: "SET_ROLES", payload: response.data.roles });
      } else {
        console.warn(
          "[AuthContext] fetchUserRoles: response.data.roles not found or not an array, setting empty roles. Response:",
          response.data,
        );
  ...
  useEffect(() => {
    console.log(
      "[AuthContext] Initial useEffect RUNNING. isLoadingUser:",
      state.isLoadingUser,
    );
    ...
    if (user) {
      ...
      if (user.user?.id) {
        ...
        console.warn(
          "[AuthContext] User found in localStorage but without an ID. Not fetching roles.",
        );
        dispatch({ type: "SET_LOADING", payload: false });
      }
    } else {
      console.log(
        "[AuthContext] No user in localStorage, setting loading to false.",
      );
      dispatch({ type: "SET_LOADING", payload: false });
    }
  ...
  const login = async (loginData: LoginRequestDTO) => {
    try {
      ...
      if (userData.user?.id) {
        console.log(
          "[AuthContext] Fetching roles after login for user:",
          userData.user.id,
        );
