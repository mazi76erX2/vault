...
export const calculateRelativeLuminance = (hexColor: string): number => {
  try {
    ...
    if (!/^[0-9A-Fa-f]{6}$/.test(fullHex)) {
      console.warn(
        `Invalid hex color for luminance: ${hexColor}, defaulting to mid-luminance`,
      );
      return 0.5;
    }
...
export const getContrastColor = (
  ...
  try {
    const bgWithHash = backgroundColor.startsWith("#")
      ? backgroundColor
      : `#${backgroundColor}`;
...
export const calculateContrastRatio = (
  ...
  try {
    ...
    console.error(
      `Error calculating contrast ratio between ${color1} and ${color2}:`,
      error,
    );
    return 1;
  }
